...
def plot_observations_municipalities_candidates(df):
    ...
    for i in range(len(nyears)):
        plt.text(
            x=nyears[i] * 3 - width / 4,
            y=df_main_dataset.groupby("jahr").gkz.nunique()[label[i]] + 5,
            s=df_main_dataset.groupby("jahr").gkz.nunique()[label[i]],
        )
    ...
    plt.bar(
        x=nyears * 3 - width / 4,
        height=df_main_dataset.loc[df_main_dataset["female"] == 1]
        .groupby("jahr")
        .gewinn_norm.count(),
        width=width / 2,
        label="females",
    ...
    for i in range(len(nyears)):
        plt.text(
            x=nyears[i] * 3 - width / 1.5,
            y=df_main_dataset.loc[df_main_dataset["female"] == 1]
            .groupby("jahr")
            .gewinn_norm.count()[label[i]]
            + 200,
            s=df_main_dataset.loc[df_main_dataset["female"] == 1]
            .groupby("jahr")
            .gewinn_norm.count()[label[i]],
        )
    ...
    plt.bar(
        x=nyears * 3 + width / 4,
        height=df_main_dataset.loc[df_main_dataset["female"] == 0]
        .groupby("jahr")
        .gewinn_norm.count(),
        width=width / 2,
        tick_label=df_main_dataset.jahr.unique().astype(int),
    ...
    for i in range(len(nyears)):
        plt.text(
            x=nyears[i] * 3 - width / 4,
            y=df_main_dataset.loc[df_main_dataset["female"] == 0]
            .groupby("jahr")
            .gewinn_norm.count()[label[i]]
            + 200,
            s=df_main_dataset.loc[df_main_dataset["female"] == 0]
            .groupby("jahr")
            .gewinn_norm.count()[label[i]],
        )
...
def summary_stats(df):
    ...
    measures = np.array(["Count", "Mean", "SD", "Min", "Max"])

    arrays = [
        np.repeat(a=np.array(["All candidates", "Female candidates"]), repeats=5),
        np.tile(measures, 2),
    ]
